...
public class OperatorRun {
	...
	public static void main(String[] args) {
		...
		boolean r2 = (1<=num) && (num<=100);
		System.out.println("num의 값은 1이상 100이하의 수인가? : "+r2);
		// && : ~ 이고, 그리고, ~하면서
		
		char yn = 'y';
		
		// yn 변수에 저장된 값이 'y' 이거나 'Y'인 경우 판별
...
//		str = str + ", Java!";
		...
		System.out.println(random + "은(는) "+str2+"입니다.");
		
		// 랜덤값을 문자로 변환했을 때 대문자인지 아닌지 판별
		
		
		String str3 = 'A' <= random && random <= 'Z' ? "대문자임" : "대문자 아님";
		System.out.println(random +"을(를) 문자로 변환한 값은 " +(char)random+ "이고 " + str3+ "니다.");
